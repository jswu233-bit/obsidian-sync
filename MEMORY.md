...
- Discord 群聊里，同一问题/同一 @ 触发默认只回复一次；若已完成作答，后续重复投递优先静默去重，避免重复回复。

### [CASE-20260616-01] 场景纠正不要直接泛化
- 场景：用户在当前会话里用短句纠正入口/场景判断（例如“不能群聊”）。
- 判断：先核对当前会话的 `channel`、`surface`、`chat_type` 和 metadata，再决定是否提升为跨场景长期规则。
- 动作：把这类短句优先记为“当前场景纠正”；只有当同一约束在多个场景稳定成立时，才升级为长期记忆。
- 验收：不再把当前入口纠正误扩成泛化的群聊/频道规则。
- 回退：如果后续出现稳定跨场景证据，再重新评估是否纳入长期规则。
- type: insight
- temp: H
...
### [CASE-20260601-01] Skill/SOP 按 SkillOpt 式闭环迭代
- 场景：高频任务（如日报、搜索 SOP、Git 同步、OpenClaw 自动化）需要持续提升稳定性与效果时。
- 判断：不要只凭感觉改 `SKILL.md` / SOP；应把 Skill 文档视为可迭代、可验证、可回滚的外部参数。
- 动作：采用“任务样本 → 运行 → 失败分析 → 修改 skill/SOP → 留出验证”的闭环；每次修改保留验证样本与回滚路径。
- 验收：修改后在留出样本上通过，且错误率、人工干预次数或交付质量至少有一项明确改善。
- 回退：若验证失败，回退到上一版 skill/SOP，仅保留失败分析到 `.learnings/`。
- type: insight
- temp: H
...
- Discord `#ll` 频道总规则（写入：2026-04-12）：**@谁，谁说话**。未被 @ 的 agent 保持静默，也不跨 bot 代答。
